...
export const JobProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (user) {
      ...
      if (user.role === "ROLE_JOB_SEEKER") {
        ...
        const loadAppliedJobs = async () => {
          try {
            ...
            const localAppliedJobs = localStorage.getItem(
              `appliedJobs_${user.userId}`,
            );
            if (localAppliedJobs) {
              try {
                ...
                console.error(
                  "Error loading applied jobs from localStorage:",
                  err,
                );
              }
            }
        ...
        const loadSavedJobs = async () => {
          try {
            ...
            // Fallback to localStorage if backend fails
            const localSavedJobs = localStorage.getItem(
              `savedJobs_${user.userId}`,
            );
            if (localSavedJobs) {
              try {
                ...
                console.error(
                  "Error loading saved jobs from localStorage:",
                  err,
                );
              }
            }
      ...
      if (user.role === "ROLE_EMPLOYER") {
        const savedPostedJobs = localStorage.getItem(
          `postedJobs_${user.userId || user.id}`,
        );
        const savedAllApplications = localStorage.getItem(
          `allApplications_${user.userId || user.id}`,
        );
  ...
  const postJob = (jobData) => {
    ...
    const existingGlobalJobs = JSON.parse(
      localStorage.getItem("globalPostedJobs") || "[]",
    );
    localStorage.setItem(
      "globalPostedJobs",
      JSON.stringify([...existingGlobalJobs, newJob]),
    );
  ...
  const deleteJob = (jobId) => {
    ...
    const updatedGlobalJobs = existingGlobalJobs.filter(
      (job) => job.id !== jobId,
    );
  ...
  const getJobApplications = (jobId) => {
    const globalApplications = JSON.parse(
      localStorage.getItem("globalApplications") || "[]",
    );
  ...
  const getAllJobsSync = (staticJobs) => {
    const globalPostedJobs = JSON.parse(
      localStorage.getItem("globalPostedJobs") || "[]",
    );
