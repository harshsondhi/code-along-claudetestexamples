...
const PostJob = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const requirementsList = formData.requirements.filter((req) =>
        req.trim(),
      );
      const benefitsList = formData.benefits.filter((benefit) =>
        benefit.trim(),
      );
      ...
      const existingEmployerJobs = JSON.parse(
        localStorage.getItem(employerKey) || "[]",
      );
      localStorage.setItem(
        employerKey,
        JSON.stringify([...existingEmployerJobs, newJob]),
      );
